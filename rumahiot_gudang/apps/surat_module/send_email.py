...
# Send the notification using different lamba execution
@task
def send_device_notification_email_worker(user_uuid, user_sensor_uuid, device_uuid, email, device_name,
                                          user_sensor_name,
    ...
    logger.log_device_notification(user_uuid=user_uuid, user_sensor_uuid=user_sensor_uuid, device_uuid=device_uuid,
                                   device_name=device_name,
                                   user_sensor_name=user_sensor_name, threshold_value=threshold_value,
                                   latest_value=latest_value,
                                   time_reached=time_reached, threshold_direction=threshold_direction,
                                   unit_symbol=unit_symbol,
...
def send_device_android_notification_worker(user_uuid, device_uuid, user_sensor_name, device_name,
                                            user_sensor_uuid, status, time_reached):
    # header = {
    ...
    #     'device_name': device_name,
    #     'user_sensor_uuid': user_sensor_name,
    #     'user_sensor_name'   : user_sensor_uuid,
    #     'status': status,
    #     'timestamp': timestamp
    # }
